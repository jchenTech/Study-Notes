...
![img](https://gitee.com/jchenTech/images/raw/master/img/20201130205006.jpeg)
...
![img](https://gitee.com/jchenTech/images/raw/master/img/20201130210324.jpeg)

> 上手体验一下，在home目录下测试

如果你想要使用 vim 来建立一个名为 jchenstudy.txt 的文件时，你可以这样做：

```
[root@localhost home]# vim jchenstudy.txt
```
...
```
# 此命令创建了一个用户jchen，其中-m选项用来为登录名kuangshen产生一个主目录 /home/jchen
[root@localhost home]# useradd -m jchen
```
...
此命令将用户jchen的登录Shell修改为ksh，主目录改为/home/z，用户组改为developer。
...
此命令将用户 jchen 的口令删除，这样用户 jchen下一次登录时，系统就不再允许该用户登录了。
